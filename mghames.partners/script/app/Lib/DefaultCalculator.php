...
class DefaultCalculator implements Calculator
{
    public static function getAttribute(string $attribute, CartItem $cartItem)
    {
        ...
        switch ($attribute) {
            ...
            case 'discountTotal':
                return round($cartItem->discount * (int)$cartItem->qty, $decimals);
            case 'priceTotal':
                return round($cartItem->price * (int)$cartItem->qty, $decimals);
            case 'subtotal':
                return max(round($cartItem->priceTotal - $cartItem->discountTotal, $decimals), 0);
            case 'priceTarget':
                return round(($cartItem->priceTotal - $cartItem->discountTotal) / (int)$cartItem->qty, $decimals);
            case 'taxTotal':
