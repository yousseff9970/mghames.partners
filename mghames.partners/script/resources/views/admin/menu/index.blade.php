				...
				<div class="row mb-10" >
					...
					<div class="col-lg-6">
						<div class="save-menu float-right" >
							<form class="ajaxform" class="float-right" method="post" action="{{ route('admin.menus.MenuNodeStore') }}"> @csrf <input type="hidden" name="data" id="data"> <input type="hidden" name="menu_id" value="{{ $info->id }}"> <button id="form-button" class="btn btn-primary basicbtn" type="submit">{{ __('Save Changes') }}</button></form>
						</div>
					</div>
				</div>
