...
@section('head')
@include('layouts.backend.partials.headersection',['title'=>'Edit Admin','prev'=> url('seller/admin')])
@endsection
...
<div class="row">
	<div class="col-lg-9">
		<div class="card">
			<div class="card-body">
				<h4>{{ __('Edit Admin') }}</h4>
				<form method="post" action="{{ route('seller.admin.update',$user->id) }}" class="ajaxform">
                    @csrf
                    @method('PUT')
					<div class="pt-20">
						@php
                        ...
                        <div class="form-group">
                            <label for="roles">{{ __('Assign Permissions') }}</label>
                                <select required name="roles[]" id="roles" class="form-control select2" multiple>
                                    ...
                                    @foreach ($roles as $role)
                                        <option value="{{ $role }}" @if(in_array($role,$permissions)) selected="" @endif>{{ str_replace('_',' ',$role) }}</option>
                                    @endforeach
                                </select>
                            </div>
                        <div class="form-group">
